...
class TransformerPipe:
    ...
    def create_median_price(self):
        price_series = self.df.price_before_discount
        median_array = (self.df.price_min.to_numpy(dtype='int')
                        + self.df.price_max.to_numpy(dtype='int')) / 2
        median_array = median_array.astype('int')
        return self.df.assign(price_median=median_array)

    def create_pages(self, total_page=120, items_per_page=25):
        pages = []
        j = 0
    ...
    def fix_fill_names(self) -> pd.DataFrame:
        ...
        path_cwd = Path.cwd().parents[0]
        reference_path = path_cwd / "data" / "brand_reference" / "master_brand_names_revised.json"
        reference = pd.read_json(reference_path, typ='series')
...
def match_brand_name(value, ref):
    for brand in ref:
        try:
            if re.search(brand, value):
                fill_name = re.search(brand, value).group()
                return fill_name
        except TypeError:
            continue
